...
class ctl_news
{
   ...
   /**
    * https://www.yiabs.com/yuanma.html首页
    * 
    */
	...
	public function search(){
		...
		if($q=='' || $q=='请输入关键字搜索'){
			die("<script> alert('请输入关键字');parent.location.href='http://sm.nedor.cn/';</script>");
		}
		...
		if (!db::num_rows()){
				die("<script> alert('没有相关内容');parent.location.href='http://sm.nedor.cn/baike';</script>");
        }else{
				...
				/********* KDT *************/
				$seo_header['title']       = $q.'的命理知识，关于'.$q.'的百科知识-易读网';
				$seo_header['keywords']    = $q;
				$seo_header['description'] = '易读网为你提供【'.$q.'】的命理知识，为您提供最新最全的【'.$q.'】的百科知识【'.$q.'】。百科知识尽在易读网。';
