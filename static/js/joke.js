...
(function(){
  ...
  $(".loadMore").click(function(){
	var btext = $(this).find("b").html();
	var bdiv = $(this).parents(".longPic");
    var s_top = bdiv.offset().top;
	var imghei = $(this).prev("span").find("img").height();
	imghei += 30;
	if(btext=='点击收回'){
			bdiv.css({"height":"600px","overflow":"hidden"});
			$(this).css({"top":"570px","bottom":""});
			$(this).find("b").css("background-position","-30px -348px");
			$(this).find("b").html("加载更多");
            $(window).scrollTop(s_top);
		  }
		  else{
			bdiv.css({"height":imghei,"overflow":"visible"});
			$(this).css({"top":"","bottom":0});
			$(this).find("b").css("background-position","-145px -348px");
			$(this).find("b").html("点击收回");
			return false;
		  }
    return false;
  })
  
  $(".itemPic img").click(function(){
	  var bdiv = $(this).parents(".longPic");
      var s_top = bdiv.offset().top;
	  var spanbtn = bdiv.find("span.loadMore");
	  if(spanbtn.find("b").html()=="加载更多"){
	  	var _href = $(this).parents("a").attr("href");
		loaction=_href;
	  	return;
	  }
	  event.preventDefault();
	  bdiv.css({"height":"600px","overflow":"hidden"});
	  spanbtn.css({"top":"570px","bottom":""});
	  spanbtn.find("b").css("background-position","-30px -348px");
	  spanbtn.find("b").html("加载更多");
      $(window).scrollTop(s_top);
  });
