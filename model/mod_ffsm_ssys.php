...
class mod_ffsm_ssys {
	...
	public static function ssys($row){
	...
	public static function ssysdouble($row){
		...
		$cookies2 = mod_wuhangbazi::get_bzwh($row['girl_username'],$row['girl_username'],$sex[$row['gender2']],$row['y2'],$row['m2'],$row['d2'],$row['h2'],$row['i'],$row['cY'],$row['cM'],$row['cD'],$row['cH'],$row['term1'],$row['term2'],$row['start_term'],$row['end_term'],$row['start_term1'],$row['end_term1'],$row['lDate']);
        $info = self::public_sm($cookies);
	...
	public static function ssysdata($scope,$data_y,$data_m,$data_d,$data_h,$oid,$amount=0){
		$date_time=date_create($data_y."-".$data_m."-".$data_d." ".$data_h.":00:00");
		$times=date_timestamp_get($date_time);
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbQ=='));
  ...
  //print_r($data);
 	...
 	public static function ssnvdata($scope,$data_y,$data_m,$data_d,$data_h,$data_y2,$data_m2,$data_d2,$data_h2,$oid,$amount=0){
		$date_time=date_create($data_y."-".$data_m."-".$data_d." ".$data_h.":00:00");
		$date_time2=date_create($data_y2."-".$data_m2."-".$data_d2." ".$data_h2.":00:00");
		$times=date_timestamp_get($date_time);
		$times2=date_timestamp_get($date_time2);
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9kb3VibGVkYXRh')); 
		curl_setopt($curl, CURLOPT_HEADER,0);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_POST, 1);
  ...
  //print_r($data);
	...
	public static function ssysdata_auth(){
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9hdXRo'));
		curl_setopt($curl, CURLOPT_HEADER,0);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_POST, 1);
		$post_data = array(
			"agentid" => "20200818",
          	"return_uri"=>urlencode($_SERVER['HTTP_HOST'])
			);
		curl_setopt($curl, CURLOPT_POSTFIELDS, $post_data);
		$data = curl_exec($curl);
		curl_close($curl);
	...
	public static function ssysdata_celebrity(){
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9jZWxlYnJpdHk='));
		curl_setopt($curl, CURLOPT_HEADER,0);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_POST, 1);
	...
	public static function ssysdata_faq($type){
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9mYXE='));
		curl_setopt($curl, CURLOPT_HEADER,0);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_POST, 1);
  	...
  	public static function ssysdata_share($type){
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9zaGFyZQ=='));
		curl_setopt($curl, CURLOPT_HEADER,0);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_POST, 1);
	...
	public static function ssysdata_celebrity_ys($cbid){
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9jZWxlYnJpdHlfeXM='));
		curl_setopt($curl, CURLOPT_HEADER,0);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_POST, 1);
...
6、吉利楼层末位数为：0 5 <br>';
    ...
    function bazipp($row){
		...
		$arrgs=array("甲"=>1,"乙"=>2,"丙"=>3,"丁"=>4,"戊"=>5,"己"=>6,"庚"=>7,"辛"=>8,"壬"=>9,"癸"=>10);

$arrzs=array("子"=>1,"丑"=>2,"寅"=>3,"卯"=>4,"辰"=>5,"巳"=>6,"午"=>7,"未"=>8,"申"=>9,"酉"=>10,"戌"=>11,"亥"=>12);
        $taiyuan=cls_paipan::getTaiyuan($mgx,$mzx);
        $minggong=cls_paipan::getMinggong($mzx,$hzx,$ygx);//胎元
        $shengong=cls_paipan::getShengong($mgx,$mzx,$hzx);//胎元
        ...
        $shishen5 = cls_paipan::getShishen($arrgs[substr($taiyuan,0,3)],$rgx);
        $shishen6 = cls_paipan::getShishen($arrgs[substr($minggong,0,3)],$rgx);
        $shishen7 = cls_paipan::getShishen($arrgs[substr($shengong,0,3)],$rgx);
		
		$z_shishen1=cls_paipan::getShishen_zs($yzx,$rgx);
		$z_shishen2=cls_paipan::getShishen_zs($mzx,$rgx);
		$z_shishen3=cls_paipan::getShishen_zs($rzx,$rgx);
		$z_shishen4=cls_paipan::getShishen_zs($hzx,$rgx);
		$z_shishen5=cls_paipan::getShishen_zs($arrzs[substr($taiyuan,3,3)],$rgx);
		$z_shishen6=cls_paipan::getShishen_zs($arrzs[substr($minggong,3,3)],$rgx);
		$z_shishen7=cls_paipan::getShishen_zs($arrzs[substr($shengong,3,3)],$rgx);
        ...
        $zanggan3 = cls_paipan::getDcang($rzx);
        $zanggan4 = $hcg1.$hcg2.$hcg3;
        $zanggan5 = cls_paipan::getDcang($arrzs[substr($taiyuan,3,3)]);
        $zanggan6 = cls_paipan::getDcang($arrzs[substr($minggong,3,3)]);
        $zanggan7 = cls_paipan::getDcang($arrzs[substr($shengong,3,3)]);
        ...
        $nayin5 = cls_paipan::getNayin($taiyuan);
        $nayin6 = cls_paipan::getNayin($minggong);
        $nayin7 = cls_paipan::getNayin($shengong);
		$wuxing=array();
		$wuxing['yg']=cls_paipan::getWuxing($yg);
		$wuxing['yz']=cls_paipan::getWuxing($yz);
		$wuxing['mg']=cls_paipan::getWuxing($mg);
		$wuxing['mz']=cls_paipan::getWuxing($mz);
		$wuxing['rg']=cls_paipan::getWuxing($rg);
		$wuxing['rz']=cls_paipan::getWuxing($rz);
		$wuxing['hg']=cls_paipan::getWuxing($hg);
		$wuxing['hz']=cls_paipan::getWuxing($hz);
		$wuxing['tyg']=cls_paipan::getWuxing(substr($taiyuan,0,3));
		$wuxing['tyz']=cls_paipan::getWuxing(substr($taiyuan,3,3));
		$wuxing['mgg']=cls_paipan::getWuxing(substr($minggong,0,3));
		$wuxing['mgz']=cls_paipan::getWuxing(substr($minggong,3,3));
		$wuxing['sgg']=cls_paipan::getWuxing(substr($shengong,0,3));
		$wuxing['sgz']=cls_paipan::getWuxing(substr($shengong,3,3));

        $data_Array = array('taiyuan'=>$taiyuan,'nayintaiyuan'=>$nayintaiyuan,'minggong'=>$minggong,'zysj'=>$zysj,'sub_zysj'=>substr($zysj,3,1),'shishen'=>$shishen,'riyuan'=>$riyuan,'sizhu'=>$sizhu,'jqstr'=>$jqstr,'shishen1'=>$shishen1,'shishen2'=>$shishen2,'shishen3'=>$shishen3,'shishen4'=>$shishen4,'shishen5'=>$shishen5,'shishen6'=>$shishen6,'shishen7'=>$shishen7,'z_shishen1'=>$z_shishen1,'z_shishen2'=>$z_shishen2,'z_shishen3'=>$z_shishen3,'z_shishen4'=>$z_shishen4,'z_shishen5'=>$z_shishen5,'z_shishen6'=>$z_shishen6,'z_shishen7'=>$z_shishen7,'yg'=>$yg,'mg'=>$mg,'rg'=>$rg,'hg'=>$hg,'yz'=>$yz,'mz'=>$mz,'rz'=>$rz,'hz'=>$hz,'taiyuan_g'=>substr($taiyuan,0,3),'taiyuan_z'=>substr($taiyuan,3,3),'minggong_g'=>substr($minggong,0,3),'minggong_z'=>substr($minggong,3,3),'shengong_g'=>substr($shengong,0,3),'shengong_z'=>substr($shengong,3,3),'xkrgz'=>cls_paipan::xunkong($rgz),'nayinminggong'=>$nayinminggong,'zanggan1'=>$zanggan1,'zanggan2'=>$zanggan2,'zanggan3'=>$zanggan3,'zanggan4'=>$zanggan4,'zanggan5'=>$zanggan5,'zanggan6'=>$zanggan6,'zanggan7'=>$zanggan7,'nayin1'=>$nayin1,'nayin2'=>$nayin2,'nayin3'=>$nayin3,'nayin4'=>$nayin4,'nayin5'=>$nayin5,'nayin6'=>$nayin6,'nayin7'=>$nayin7,'jqstr'=>$jqstr,'wuxing'=>$wuxing);
        return $data_Array;
    }
