...
class mod_xingzuo { 
	...
	public function get_date_yunshi($tid,$date){
		$data = file_get_contents('http://cqapi.xlibbs.com/?ct=api&ac=xingzuo_yunshi_ajax&date='.$date.'&tid='.$tid);
	...
	public static function get_yunshi($tid){
		...
		//今天和明天运势采用自己数据
		$data_jt = file_get_contents('http://cqapi.xlibbs.com/?ct=api&ac=xingzuo_yunshi_ajax&date='.$jintian.'&tid='.$tid);
		...
		$data_mt = file_get_contents('http://cqapi.xlibbs.com/?ct=api&ac=xingzuo_yunshi_ajax&date='.$mingtian.'&tid='.$tid);
