...
function debug_format_errmsg($log_type='debug', $errno, $errmsg, $filename, $linenum, $vars)
{
    ...
    //$err .= "<strong>HUAHUO框架应用错误跟踪：</strong><br />\n";
    ...
    $err .= "断点：<font color='#747267'>{$error_line}</font><br />\n";
    $err .= "详细跟踪：<br />\n";
...
function show_debug_error()
{
    ...
    if( $_debug_error_msg != '' || $_debug_mt_info !='')
    {
        if( (OPEN_DEBUG === true || $_debug_safe_ip===true) && !$_debug_hidden )
        {
            ...
            echo '<div style="line-height:24px; background: #FBFEEF;;"><div style="float:left"><strong>HUAHUO框架应用错误/警告信息追踪：</strong></div><div style="float:right"><a href="#" onclick="javascript:debug_close_all();">[关闭全部]</a></div>';
