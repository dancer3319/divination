...
class cls_paipan
{
	...
	$zzu=array("乙丑","丁卯","己巳","辛未","癸酉","乙亥","丁丑","己卯","辛巳","癸未","乙酉", "丁亥","己丑","辛卯","癸巳","乙未","丁酉", "己亥","辛丑","癸卯","乙巳","丁未","己酉","辛亥","癸丑","乙卯","丁巳","己未","辛酉","癸亥");
	...
	function getShishen_zs($dz,$rg)
	{
		$ycg=cls_paipan::getDcang($dz);
		$tiangan=array("癸","甲","乙","丙","丁","戊","己","庚","辛","壬");
            $ycg1=substr($ycg,0,3);
            $ycg1x=array_search($ycg1,$tiangan);
            $row=cls_paipan::getShishen($ycg1x,$rg);

            if(substr($ycg,3,3)!=''){
                $ycg2=substr($ycg,3,3);
                $ycg2x=array_search($ycg2,$tiangan);
                $row.="<br>".cls_paipan::getShishen($ycg2x,$rg);
            }
            if(substr($ycg,6,3)!=''){
                $ycg3=substr($ycg,6,3);
                $ycg3x=array_search($ycg3,$tiangan);
                $row.="<br>".cls_paipan::getShishen($ycg3x,$rg);
            }
			return $row;
	}
