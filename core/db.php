...
class db{
    ...
    */
    ...
    private static function _init_mysql( $is_master = false )
    {
        ...
        //创建连接 array('default'=>array('w'=>array()));
		$str_links = self::$links;
		$str_line_name = self::$link_name;		
		
        if(empty($str_links[$str_line_name][$link])){
            try{
                $str_links[$str_line_name][$link] = mysqli_connect($host, $db_config['user'], $db_config['pass']);				
                if( empty($str_links[$str_line_name][$link]) ){
                    throw new Exception(mysqli_error());
                }else{
                    $charset = str_replace('-', '', strtolower($db_config['charset']));
                    mysqli_query($str_links[$str_line_name][$link]," SET character_set_connection=" . $charset . ", character_set_results=" . $charset . ", character_set_client=binary, sql_mode='' ");
                    if (mysqli_select_db($str_links[$str_line_name][$link],$db_config['name']) === false ) {
        ...
        self::$cur_link = $str_links[$str_line_name][$link];
        return $str_links[$str_line_name][$link];
    }
    ...
    private static function _get_rsid( $rsid='' ){
        ...
        $rsid_int = intval(json_encode($rsid));
    ...
    public static function query ($sql, $is_master = false){
        ...
        try{
            self::$cur_result = mysqli_query(self::$cur_link,$sql);			
            //把结果集 Resource ID#21 转成 整型21,否则PHP 5.3 5.4 报错，因为在5.3 5.4里面结果集不能作为数字下标
            $rsid_int = intval(json_encode(self::$cur_result));
			//print_r(self::$cur_result);exit;
